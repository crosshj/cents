...
export async function setupAccounts({
	...
	window.timeWeightedAverage.saveHistory = async (accountTitle, entries) => {
		const rawAccounts = await GetData('accountsRawData');
		const account = rawAccounts.liabilities.find(
			(account) =>
				account?.title?.toLowerCase() === accountTitle.toLowerCase()
		);
		if (!account) return;
		account.amount =
			'[time-weighted-average]\n' +
			entries.map(({ date, amount }) => `${amount}:${date}`).join(', ');
		await updateAndSaveAccounts(
			auth.supabase,
			session.user.id,
			rawAccounts
		);
