...
function parseTimeWeightedAverage(rawAccounts) {
	...
	for (const type of ['liabilities', 'assets']) {
		for (const item of rawAccounts[type] || []) {
			...
			for (const key of Object.keys(item)) {
				...
				const [firstLine, ...lines] = item[key].trim().split(/\r?\n/);
				...
				parsed[key + '_type'] = 'time-weighted-average';
				// entries.sort((a, b) => a.d - b.d);
				// let w = 0,
				// 	t = 0;
				// for (let i = 0; i < entries.length; i++) {
				// 	const next = entries[i + 1]?.d || new Date();
				// 	const days = (next - entries[i].d) / 86400000;
				// 	w += entries[i].v * days;
				// 	t += days;
				// }
				// parsed[key] = t ? w / t : entries.at(-1).v;

				parsed[key] =
					entries.reduce((sum, e) => sum + e.v, 0) / entries.length;
			}
			parsedAccounts[type].push(parsed);
		}
	}
...
const timeWeightedAverage = {
	inputCoerce,
	parse: parseTimeWeightedAverage,
};
...
const getAccounts = async (supabase, userId) => {
	...
	try {
		...
		const accountsWithTWA = timeWeightedAverage.parse(accountsData);
